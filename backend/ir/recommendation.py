...
def get_sim_scores(search_tea, search_description, k):
    ...
    if search_tea:
        ...
        about_tfidf, about_query_tfidf = create_vector(
            [d["about"] for d in tea_data], [search_data["about"]])
        review_tfidf, review_query_tfidf = create_vector(
            [" ".join(d["reviews"]) for d in tea_data], [" ".join(search_data["reviews"])])
        about_sims, review_sims = cosine_sims(
            about_tfidf, review_tfidf, about_query_tfidf, review_query_tfidf)
    ...
    if search_description:
        about_tfidf, about_query_tfidf = create_vector(
            [d["about"] for d in tea_data], [search_description])
        review_tfidf, review_query_tfidf = create_vector(
            [" ".join(d["reviews"]) for d in tea_data], [search_description])
        about_sims, review_sims = cosine_sims(
            about_tfidf, review_tfidf, about_query_tfidf, review_query_tfidf)
        description_merged_sims = about_weight * \
            about_sims + review_weight * review_sims
        if not search_tea:
...
def get_recommendations(search_tea, search_description, k):
    sim_scores, search_tea = get_sim_scores(
        search_tea=search_tea, search_description=search_description, k=k)
    ranked_ids = (-sim_scores).argsort()
    if search_tea != []:
        # remove the current search
        ranked_ids = ranked_ids[ranked_ids != tea_to_index[search_tea[0]]]
